...
import { FiArrowLeft, FiCamera, FiEye, FiEyeOff, FiTrash2, FiLoader } from 'react-icons/fi';
import { useAuth } from '../hooks/useAuth';

// Daftar avatar bawaan baru yang lebih menarik
...
const AccountSettingsPage = ({ onNavigate }) => {
    ...
    const handleSaveChanges = async (e) => {
        ...
        // Jika avatar preview adalah salah satu dari default, kirim URL-nya
    ...
    const handleSelectDefaultAvatar = (url) => {
        ...
        setAvatarFile(null); // Hapus file jika user memilih avatar default
        // Hapus 'DELETE' flag jika sebelumnya di-set
        if (formData.avatar === 'DELETE') {
    ...
    if (!user || !avatarPreview) {
        return <div className="min-h-screen bg-gray-100 flex justify-center items-center">Memuat pengaturan...</div>;
    }
                    ...
                    <form onSubmit={handleSaveChanges} className="bg-white p-6 md:p-8 rounded-2xl shadow-md">
                        <div className="flex flex-col items-center mb-8">
                            <div className="relative mb-4">
                                ...
                                <button type="button" onClick={handleUploadClick} className="absolute -right-2 bottom-0 bg-sesm-teal text-white p-2 rounded-full border-2 border-white shadow-md hover:bg-sesm-deep transition-colors" title="Unggah foto profil"><FiCamera size={18} /></button>
                                <button type="button" onClick={handleDeleteAvatar} className={`absolute -left-2 bottom-0 p-2 rounded-full border-2 border-white shadow-md transition-colors bg-red-500 text-white hover:bg-red-700`} title="Hapus foto profil"><FiTrash2 size={18} /></button>
                            </div>
                            
                            <h3 className="font-bold text-gray-700 mb-3">Pilih Avatar Bawaan</h3>
                            <div className="flex justify-center items-center flex-wrap gap-3">
                                {defaultAvatars.map((avatarUrl, index) => (
                                    <motion.img key={index} src={avatarUrl} alt={`Avatar ${index + 1}`} className={`w-12 h-12 rounded-full cursor-pointer border-2 transition-all ${avatarPreview === avatarUrl ? 'border-sesm-teal scale-110' : 'border-gray-200'}`} onClick={() => handleSelectDefaultAvatar(avatarUrl)} whileHover={{ scale: 1.1 }}/>
                                ))}
                            </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="pt-4"><motion.button type="submit" className="w-full bg-sesm-deep text-white font-bold py-3 rounded-lg shadow-lg hover:bg-opacity-90 disabled:bg-gray-400 flex items-center justify-center" whileTap={{ scale: 0.98 }} disabled={isSaving}>{isSaving ? <><FiLoader className="animate-spin mr-2"/> Menyimpan...</> : 'Simpan Perubahan'}</motion.button></div>
                        </div>
                    </form>
                </motion.div>
            </main>
