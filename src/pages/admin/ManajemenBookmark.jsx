...
import { FiPlus, FiFileText, FiTrash2, FiLoader, FiAlertCircle, FiEdit, FiSave, FiX, FiBookmark, FiTrendingUp, FiThumbsUp, FiThumbsDown, FiUpload, FiLink } from 'react-icons/fi';
import BookmarkService from '../../services/bookmarkService';
import Notification from '../../components/Notification';

// --- Komponen Modal Penilaian (Tidak Berubah) ---
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-2xl w-full max-w-2xl shadow-xl flex flex-col">
                <form onSubmit={handleSubmit}>
                    ...
                    <footer className="bg-gray-50 p-4 flex justify-end gap-3 rounded-b-2xl border-t"><button type="button" onClick={onClose} className="px-5 py-2">Batal</button><button type="submit" disabled={isSaving} className="px-5 py-2 bg-sesm-deep text-white rounded-lg flex items-center gap-2"><FiSave/>{isSaving ? 'Menyimpan...' : 'Simpan'}</button></footer>
                </form>
            </motion.div>
        </motion.div>
    );
...
const ManajemenBookmark = () => {
    ...
    const handleSave = async (data, id) => {
        try {
            ...
            setNotif({ isOpen: true, message: error.response?.data?.message || "Gagal menyimpan.", success: false });
        }
    };
                            ...
                            <div>
                                ...
                                {bookmarks.length === 0 ? <div className="text-center py-16 text-gray-400"><FiAlertCircle size={48} className="mx-auto"/><p>Belum ada materi.</p></div> : (
                                    <table className="w-full text-sm">
                                        <thead className="text-xs text-gray-700 uppercase bg-gray-100"><tr><th className="px-6 py-3">Judul</th><th className="px-6 py-3">Mapel</th><th className="px-6 py-3">Jumlah Soal</th><th className="px-6 py-3">Penilaian</th><th className="px-6 py-3 text-center">Aksi</th></tr></thead>
                                        <tbody>{bookmarks.map(item => ( <tr key={item.id} className="hover:bg-gray-50 border-b">
                                            ...
                                            <td className="px-6 py-4">{item.tasks?.length || 0}</td>
                                            <td className="px-6 py-4 capitalize">{item.grading_type}</td>
                                            <td className="px-6 py-4 flex justify-center gap-4"><button onClick={() => handleOpenModal(item)} className="text-blue-600"><FiEdit/></button><button onClick={() => handleDelete(item.id, item.title)} className="text-red-600"><FiTrash2/></button></td>
                                        </tr>))}</tbody>
                                    </table>
                                )}
                            </div>
                        ...
                        {!loading && activeTab === 'nilai' && (
                            <div>
                                ...
                                {submissions.length === 0 ? <p className="text-center text-gray-400 py-8">Belum ada siswa yang mengerjakan materi ini.</p> : (
                                    <table className="w-full text-sm">
                                        <thead className="text-xs uppercase bg-gray-100"><tr><th className="px-6 py-3">Nama Siswa</th><th className="px-6 py-3">Tanggal</th><th className="px-6 py-3">Status</th><th className="px-6 py-3">Skor</th><th className="px-6 py-3 text-center">Aksi</th></tr></thead>
                                        <tbody>{submissions.map(sub => (<tr key={sub.id} className="hover:bg-gray-50 border-b"><td className="px-6 py-4 font-bold">{sub.student_name}</td><td className="px-6 py-4">{new Date(sub.submission_date).toLocaleString('id-ID')}</td><td className="px-6 py-4"><span className={`px-2 py-1 text-xs rounded-full ${sub.status === 'dinilai' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{sub.status}</span></td><td className="px-6 py-4 font-bold">{sub.score ?? '-'}</td><td className="px-6 py-4 text-center"><button onClick={() => setSelectedSubmission(sub)} className="text-blue-600"><FiEdit/></button></td></tr>))}</tbody>
                                    </table>
                                )}
                            </div>
                        )}
