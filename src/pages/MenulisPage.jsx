...
const MenulisIcon = () => (
    <svg viewBox="0 0 64 64" className="w-12 h-12 text-orange-500">
        <rect x="10" y="10" width="44" height="44" rx="5" fill="currentColor" fillOpacity="0.1"/>
        <path d="M22 42l10-20 10 20" stroke="currentColor" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
        <path d="M27 34 h 10" stroke="currentColor" strokeWidth="3" fill="none" strokeLinecap="round"/>
    </svg>
);

// --- Data Bab ---
const menulisChapters = [
  { id: 1, title: 'Menebalkan Garis dan Bentuk', progress: 90, Icon: MenulisIcon, iconBgColor: 'bg-orange-100' },
  { id: 2, title: 'Menulis Huruf Lepas', progress: 60, Icon: MenulisIcon, iconBgColor: 'bg-orange-100' },
  { id: 3, title: 'Menyalin Kata', progress: 30, Icon: MenulisIcon, iconBgColor: 'bg-orange-100' },
  { id: 4, title: 'Menulis Nama Sendiri', progress: 10, Icon: MenulisIcon, iconBgColor: 'bg-orange-100' },
];

const MenulisPage = ({ onNavigate }) => {
  return (
      ...
      <div className="md:hidden flex flex-col h-screen">
        <header className="bg-sesm-teal pt-8 pb-4 px-6 rounded-b-[2.5rem] shadow-lg text-white">
            ...
            <div className="relative mb-4">
                ...
                <input type="text" placeholder="Cari materi menulis..." className="w-full bg-white text-gray-800 placeholder:text-gray-500 rounded-full py-3 pl-12 pr-4 text-sm border-none focus:outline-none focus:ring-2 focus:ring-white/50" />
            </div>
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold tracking-wide">MENULIS</h1>
                    <p className="text-sm opacity-90">Jenjang TK Kurikulum SESM</p>
                </div>
                <FaPencilAlt size={40} className="opacity-80"/>
            </div>
        </header>
        <main className="flex-1 overflow-y-auto pt-6 pb-6 px-6">
            <h2 className="text-lg font-bold text-gray-800 mb-4">Semua bab</h2>
            <div className="space-y-4">
              {menulisChapters.map(({ id, title, progress, Icon, iconBgColor }) => (
        ...
        <div className="w-full max-w-2xl mx-auto">
          <header className="bg-white p-6 rounded-2xl shadow-md mb-8">
              <div className="flex items-center mb-4">
                  ...
                  <div className="flex-1">
                      <h1 className="text-3xl font-bold text-sesm-deep tracking-wide">MENULIS</h1>
                      <p className="text-md text-gray-500">Jenjang TK Kurikulum SESM</p>
                  </div>
                  <FaPencilAlt size={40} className="text-sesm-teal"/>
              </div>
          </header>
          <main>
