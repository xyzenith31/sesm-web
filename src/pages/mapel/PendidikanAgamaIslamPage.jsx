...
// Data Dummy untuk Riwayat & Nilai
const dummyHistory = [
    { id: 1, title: "Rukun Iman dan Rukun Islam", date: "10 Okt 2025", score: 85, points: 120, details: [
        { q: "Ada berapa Rukun Iman?", a: "Enam", isCorrect: true },
        { q: "Sebutkan Rukun Islam yang pertama!", a: "Puasa", isCorrect: false, correctA: "Syahadat" },
    ]},
    { id: 2, title: "Kisah Nabi dan Rasul", date: "08 Okt 2025", score: 100, points: 150, details: [
        { q: "Nabi yang dikenal sabar saat sakit adalah...", a: "Nabi Ayub AS", isCorrect: true },
    ]},
];

// Komponen-komponen UI (ChapterButton, HistoryCard, DetailModal)
const ChapterButton = ({ chapter, onClick, Icon }) => (
    ...
    >
        <div className="w-16 h-16 md:w-20 md:h-20 flex items-center justify-center rounded-xl bg-green-100 flex-shrink-0">
...
const PendidikanAgamaIslamPage = ({ onNavigate, onNavigateToWorksheet }) => {
    const { user } = useAuth();
    ...
    const SUBJECT_NAME = 'Pendidikan Agama Islam';
    const HEADER_TITLE = 'AGAMA ISLAM';
    const ICON = FaMosque;
    ...
    const renderContent = () => {
        ...
        if (activeTab === 'materi') {
            ...
            return <div className="space-y-4">{chapters.map((chapter) => <ChapterButton key={chapter.id} chapter={chapter} onClick={() => onNavigateToWorksheet({ materiKey: chapter.materiKey, chapterTitle: chapter.judul, subjectName: SUBJECT_NAME, navigationKey: 'pai' })} Icon={ICON} />)}</div>;
        }
