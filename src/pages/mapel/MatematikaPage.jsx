...
const dummyHistory = [
    { id: 1, title: "Bilangan Sampai 10", date: "11 Okt 2025", score: 100, points: 150, details: [
        { q: "Angka setelah 7 adalah...", a: "8", isCorrect: true },
        { q: "Berapa hasil dari 3 + 5?", a: "8", isCorrect: true },
    ]},
];
...
const ChapterButton = ({ chapter, onClick, Icon }) => (
    ...
    >
        <div className="w-16 h-16 md:w-20 md:h-20 flex items-center justify-center rounded-xl bg-orange-100 flex-shrink-0">
...
const MatematikaPage = ({ onNavigate, onNavigateToWorksheet }) => {
    const { user } = useAuth();
    ...
    const renderContent = () => {
        ...
        if (activeTab === 'materi') {
            ...
            return <div className="space-y-4">{chapters.map((chapter) => <ChapterButton key={chapter.id} chapter={chapter} onClick={() => onNavigateToWorksheet({ materiKey: chapter.materiKey, chapterTitle: chapter.judul, subjectName: SUBJECT_NAME, navigationKey: 'matematika' })} Icon={ICON} />)}</div>;
        }
