...
const ChapterButton = ({ chapter, onClick, Icon }) => (
    ...
    >
        <div className="w-16 h-16 md:w-20 md:h-20 flex items-center justify-center rounded-xl bg-teal-100 flex-shrink-0">
...
const HistoryCard = ({ item, onSelect }) => (
    ...
    >
        <div className="flex justify-between items-start">
            <div><h4 className="font-bold text-sesm-deep text-md">{item.title}</h4><p className="text-xs text-gray-500 mt-1">{item.date}</p></div>
        ...
        <div className="mt-3 pt-3 border-t border-gray-200 flex space-x-4">
            <div className="flex items-center space-x-2"><FiTarget className="text-blue-500" /><div><p className="text-lg font-bold text-sesm-deep">{item.score}</p><p className="text-xs text-gray-500 -mt-1">Nilai</p></div></div>
...
const BerhitungPage = ({ onNavigate, onNavigateToWorksheet }) => {
    ...
    const renderContent = () => {
        ...
        if (activeTab === 'materi') {
            ...
            return <div className="space-y-4">{chapters.map((chapter) => <ChapterButton key={chapter.id} chapter={chapter} onClick={() => onNavigateToWorksheet({ materiKey: chapter.materiKey, chapterTitle: chapter.judul, subjectName: SUBJECT_NAME, navigationKey: 'berhitung' })} Icon={ICON} />)}</div>;
        }
