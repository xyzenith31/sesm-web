        ...
        <div className="mb-6 space-y-4 max-w-lg mx-auto">
            {attachments.map((item, index) => {
                ...
                if (item.type === 'file') {
                    ...
                    if (['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(fileExtension)) return <img key={key} src={fullFileUrl} alt={`Lampiran ${index+1}`} className="max-w-full mx-auto rounded-lg shadow-md" />;
...
const QuizForm = ({ quizData: initialQuizData, onCompleteQuiz }) => {
    ...
    const handleAnswerSubmit = useCallback(() => {
        ...
        if (!isCorrect && !isRedemptionRound && quizSettings.setting_allow_redemption && !redemptionAttempts[currentQuestion.id] && currentQuestion.question_type.includes('pilihan-ganda')) {
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-gray-100 flex flex-col">
                ...
                {/* Footer */}
                <footer className="p-4 bg-white flex justify-between items-center border-t"><div><p className="text-sm text-gray-500">Poin Sementara</p><p className="font-bold text-xl text-sesm-deep">{totalPointsEarned}</p></div></footer>
            </div>
        </>
    );
};
