...
const BankSoalBookmarkModal = ({ isOpen, onClose, onQuestionsAdded }) => {
    ...
    const handleAddQuestionsToBookmark = async () => {
        if (!targetBookmarkId) {
            alert("Pilih materi bookmark tujuan terlebih dahulu.");
            return;
        }
        setIsAdding(true);
        try {
            ...
            alert("Gagal menambahkan soal dari bank ke materi bookmark.");
        } finally {
            setIsAdding(false);
        }
    };
                        ...
                        <div className='w-full max-w-md space-y-6'>
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => setTargetBookmarkId(e.target.value)}
                                    className="w-full p-3 border border-gray-300 rounded-lg bg-white"
                                    disabled={targetBookmarkLoading}
                                >
                                    <option value="">{targetBookmarkLoading ? 'Memuat...' : '-- Pilih Materi Bookmark --'}</option>
                                    {allBookmarks.map(b => <option key={b.id} value={b.id}>{b.title}</option>)}
                                </select>
                            </div>
                        </div>
                    </div>
