...
const DraftQuizModal = ({ isOpen, onClose, allQuizzes, drafts: initialDrafts, onContinueQuestionDraft, onDraftDeleted }) => {
    ...
    const handleDelete = (draft, title) => {
        setDraftToDelete(draft);
        setNotif({
            isOpen: true,
            title: "Konfirmasi Hapus",
            message: `Yakin ingin menghapus draf untuk "${title}"?`,
            isConfirmation: true,
    ...
    const confirmDeleteDraft = async () => {
        ...
        const draftKey = draftToDelete.key;
        ...
        try {
            ...
            setDrafts(prevDrafts => prevDrafts.filter(d => d.key !== draftKey));
             ...
             setNotif({
                ...
                success: true,
                isConfirmation: false // Pastikan ini false
            });
        } catch (error) {
             ...
             setNotif({
                isOpen: true,
                title: "Gagal",
                message: error.response?.data?.message || "Gagal menghapus draf dari server.",
                success: false,
                isConfirmation: false // Pastikan ini false
            });
                                ...
                                {drafts.map(draft => (
                                    <div key={draft.key} className="bg-gray-50 border rounded-lg p-4 flex justify-between items-center">
                                        ...
                                        <div className="flex items-center gap-2 flex-shrink-0">
                                            {/* Panggil handleDelete saat tombol Hapus diklik */}
                                            <button onClick={() => handleDelete(draft, draft.title)} className="p-2 text-red-500 hover:bg-red-100 rounded-md" title="Hapus Draf">
                                                <FiTrash2/>
                                            </button>
