...
const QuestionCard = ({ question, index, userAnswer, onAnswerChange, isSubmitted }) => {
  ...
  const getOptionClass = (option) => {
    if (!isSubmitted) {
      return userAnswer === option ? 'bg-sesm-teal text-white' : 'bg-white hover:bg-gray-100';
    }
    ...
    >
      ...
      {question.type === 'essay' && (
        <textarea
          ...
          className={`w-full h-24 p-3 border-2 rounded-lg focus:outline-none transition-colors ${isSubmitted ? 'bg-gray-200' : 'focus:ring-2 focus:ring-sesm-teal border-gray-300'}`}
        />
      )}
    </motion.div>
  );
      ...
      >
        <header className="flex-shrink-0 flex items-center justify-between p-4 border-b">
          <div className='flex-grow'>
            <p className="text-sm font-semibold text-sesm-teal -mb-1">{materi.mapel}</p>
            <h2 className="text-lg font-bold text-sesm-deep">{materi.judul}</h2>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-4">
            {materi.questions && materi.questions.map((q, index) => (
                <QuestionCard
                    key={`${materi.judul}-${index}`}
                    question={q}
                    index={index}
        ...
        <footer className='flex-shrink-0 p-4 bg-gray-50 border-t rounded-b-2xl'>
            {isSubmitted ? (
                ...
                <motion.button
                    ...
                    className="w-full bg-sesm-deep text-white font-bold py-3 rounded-xl shadow-lg"
                    whileTap={{ scale: 0.98 }}
                >
